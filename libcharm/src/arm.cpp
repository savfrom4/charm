...
inline void Instruction::decode_data_processing(instr_t instr) {
  ...
  if (immediate) {
    ...
    std::uint32_t value = get_bits<0, 8>(instr); /* Value, bits 0-7 */
...
inline void Instruction::decode_branch(instr_t instr) {
  ...
  std::uint32_t raw_offset = get_bits<0, 24>(instr); /* Offset, bits 0-23 */
  std::uint32_t shifted_offset = raw_offset << 2;
