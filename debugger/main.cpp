...
void debugger_start(struct addrinfo *info) {
	...
	int one = 1;
	if (setsockopt(connection, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one)) <
	    0) {
	...
	while (1) {
		...
		if (!paused) {
			debugger_network_process(connection, temp_buffer, accum_buffer,
			                         length, paused, 30);
			continue;
		}
...
void debugger_execute_command(int connection, const std::string &full_command,
	...
	while (buffer_offset > 0) {
		...
		if (!bytes_written) {
			throw std::runtime_error(
			    "debugger_execute_command: connection is lost.");
		}
