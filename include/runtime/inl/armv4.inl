...
#define VALUE_OR_SHIFT(is_imm, value, shifter)                                 \
	(is_imm ? value : _shift<is_imm, shifter>())
#define VALUE_OR_REG(is_imm, value, reg) (is_imm ? value : r[reg])

#define LAYER_DBE_NEXT(x, fmt, ...)
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_add() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_adc() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_sub() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_sbc() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_cmp() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_mov() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_rsb() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_rsc() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_and() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_eor() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_orr() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_bic() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_mvn() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_tst() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_teq() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_cmn() {
	constexpr const auto &data = instr.data;
	const Word value = VALUE_OR_SHIFT(instr.is_imm, data.op2_imm, data.op2_reg);
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_mul() {
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_mla() {
	constexpr const auto &mul = instr.mul;
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_mull() {
...
template <CPUState::CRefInstr instr> inline void CPUState::arm_mlal() {
	constexpr const auto &mull = instr.mull;
...
inline void CPUState::arm_ldr(Memory &memory) {
...
template <CPUState::CRefInstr instr>
inline void CPUState::arm_str(Memory &memory) {
	constexpr const auto &data_trans = instr.data_trans;
...
template <CPUState::CRefInstr instr>
inline void CPUState::arm_ldrh(Memory &memory) {
	...
	const Word offset =
	    VALUE_OR_REG(instr.is_imm, hw_data_trans.imm, hw_data_trans.rm);
	const Word address =
...
template <CPUState::CRefInstr instr>
inline void CPUState::arm_strh(Memory &memory) {
	constexpr const auto &hw_data_trans = instr.hw_data_trans;
...
template <CPUState::CRefInstr instr>
inline void CPUState::arm_ldm(Memory &memory) {
...
template <CPUState::CRefInstr instr>
inline void CPUState::arm_stm(Memory &memory) {
	constexpr const auto &blk_data_trans = instr.blk_data_trans;
...
template <bool is_imm, CPUState::CRefShifter shifter>
inline Word CPUState::_shift() {
	...
	if constexpr (is_imm) {
		// ignore if imm
		return 0;
	} else if constexpr (shifter.type == isa::arm::Shifter::LSL) {
		if (!amount) {
			return value;
		}
