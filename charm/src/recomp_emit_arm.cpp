...
void Recompiler::emit_arm(std::ostream &os, const arm::Instruction &instr,
  ...
  case arm::InstructionGroup::BRANCH_EXCHANGE: {
    ...
    os << "JUMP(ps.r[" << REGISTER_TABLE[(int)branchex.rm] << "]);"
       << MINIFY_COMMENT(" /* bx */");
    break;
  }
...
void Recompiler::emit_arm_data_processing(std::ostream &os,
  ...
  if (instr.immediate) {
    os << OPCODE_TABLE[(int)data.op] << "("
       ...
       << REGISTER_TABLE[(int)data.rd] << MINIFY_COMMENT_COMMA(" /* rd */, ")

       << REGISTER_TABLE[(int)data.rn] << MINIFY_COMMENT_COMMA(" /* rn */, ")
  ...
  if (data.op2_reg.is_reg) {
    ...
    os << OPCODE_TABLE[(int)data.op] << "("
       ...
       << "0x" << std::hex << (uint32_t)data.op2_reg.amount_or_rs << std::dec
...
void Recompiler::emit_arm_multiply(std::ostream &os,
  ...
  os << (mul.accumulate ? "ps.arm_mla" : "ps.arm_mul") << "("
     << (instr.set_cflags ? "true" : "false")
     << MINIFY_COMMENT_COMMA(" /* set_cond */, ")
...
void Recompiler::emit_arm_multiply_long(std::ostream &os,
  ...
  os << (mul_long.accumulate ? "ps.arm_mlal" : "ps.arm_mull") << "("
     << (instr.set_cflags ? "true" : "false")
     << MINIFY_COMMENT_COMMA(" /* set_cond */, ")
     ...
     << REGISTER_TABLE[(int)mul_long.rm] << MINIFY_COMMENT_COMMA(" /* rm */, ")
...
void Recompiler::emit_arm_data_transfer(std::ostream &os,
  ...
  os << (data_trans.load ? "ps.arm_ldr(" : "ps.arm_str(")
  ...
  if (instr.immediate) {
    os << "0x" << (int)data_trans.offset_imm << MINIFY_COMMENT(" /* offset */");
  } else {
    if (data_trans.offset_reg.is_reg) {
      os << SHIFT_TABLE[(int)data_trans.offset_reg.type] << "(ps, false, ps.r["
         << REGISTER_TABLE[(int)data_trans.offset_reg.rm] << "]"
         << MINIFY_COMMENT_COMMA(" /* rm */,")

         << "ps.r[" << REGISTER_TABLE[(int)data_trans.offset_reg.amount_or_rs]
         << "]" << MINIFY_COMMENT(" /* rs */") << ")";
    } else {
      os << SHIFT_TABLE[(int)data_trans.offset_reg.type] << "(ps, false, ps.r["
         << REGISTER_TABLE[(int)data_trans.offset_reg.rm] << "]"
...
void Recompiler::emit_arm_halfword_data_transfer(std::ostream &os,
                                                 const arm::Instruction &instr,
                                                 arm::addr_t address) {
  const auto &hw_data_trans = std::get<arm::HalfWordDataTransfer>(instr.group);

  os << (hw_data_trans.load ? "ps.arm_ldrh(" : "ps.arm_strh(")
...
void Recompiler::emit_arm_block_data_transfer(std::ostream &os,
  ...
  os << (blk_data_trans.load ? "ps.arm_ldm(" : "ps.arm_stm(")
     ...
     << blk_data_trans.reg_list << std::dec << MINIFY_COMMENT(" /* reg_list */")
     << ");";
}
